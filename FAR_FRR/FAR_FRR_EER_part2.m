...
FRR_all3=zeros(3,length(threshold_range));
...
for folder_index=1:3
    ...
    for TS_index=1:number_of_TS
        ...
        TS_name=Test_Samples_List(TS_index).name(1:(length(Test_Samples_List(TS_index).name)-9));
        DS_index_no=original_index_table(TS_index,1);
        if (length(Database_Samples_List(DS_index_no).name)<length(TS_name))
            DS_name=Database_Samples_List(DS_index_no).name;
        else
            DS_name=Database_Samples_List(DS_index_no).name(1:(length(TS_name)));
        end
        th_index=1;
        for threshold=threshold_range
            if min_distance_table(TS_index,1)<threshold
                if strcmpi(DS_name,TS_name)==1
    ...
    GAR=zeros(1,length(threshold_range));
    
    for GAR_index=1:length(threshold_range)
        GAR(1,GAR_index)=mean(threshold_matching(GAR_index,:));
    end
    FRR=1-GAR;
    
    FRR_all3(folder_index,:)=FRR;
