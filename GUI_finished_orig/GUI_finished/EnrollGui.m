...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
DS_files=dir('C:\Workspace\major_project\GUI_finished\sounds\database_sounds\temp\*.wav');
TS_files=dir('C:\Workspace\major_project\GUI_finished\sounds\test_sounds\temp\*.wav');

if ~isempty(DS_files)
    for i=1:length(DS_files)
        movefile(DS_files(i).name,'C:\Workspace\major_project\GUI_finished\sounds\database_sounds\');
    end
end

if ~isempty(TS_files)
    for i=1:length(TS_files)
        movefile(TS_files(i).name,'C:\Workspace\major_project\GUI_finished\sounds\test_sounds\');
    end
end
...
saveinpath='C:\Workspace\major_project\GUI_finished\sounds\database_sounds\temp\';
