...
framesize=32*[1:10];nof_frames=length(framesize);
...
%     
%     speech_frames_colmns=overlap2(raw_speech,framesize(P),overlap_amt);
...
%copyfile('groupedmeanOFcoloumns.m',put_spectogrphs_in);
copyfile('some_meanOFcoloumns.m',put_spectogrphs_in);
cd(put_spectogrphs_in);
filelist_images=ls('*.jpg');

%blocks=11;
%ele_after_blocks=(framesize(P)-1-mod((framesize(P)-1),blocks))/blocks;

frames_limit=1*framesize(P)/2;
tot_ele=frames_limit;
allfeatures=zeros(len_flist,tot_ele);

for i=1:len_flist;
  
    hfile_spectogram=filelist_images(i,:);
    spectogram=imread(hfile_spectogram);
    [allfeatures(i,:)]=some_meanOFcoloumns(spectogram,frames_limit);
